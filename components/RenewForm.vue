<template>
  ...
  >
    <v-card
      v-if="showTutorial"
      class="rounded-0 renew-answer text-center primary--text"
    >
      <v-container>
...
export default {
  ...
  created() {
    if (this.value && this.value.renew_answer) {
      this.apiDataToForm(this.form, { ...this.value.renew_answer })
    }
  },
  methods: {
    prev() {
    ...
    save(changeTab = true) {
      ...
      this.$axios
        .$patch('/v1/contracts/' + this.value._id, { renew_answer: form })
        .then((contract) => {
          this.apiDataToForm(this.form, contract.renew_answer)
