...
const datesByYear = (year) => {
  return dates.find((date) => date.year === year)
}

const seasonByBemisphere = {
  spring: 'fall',
  summer: 'winter',
  fall: 'spring',
  winter: 'summer',
...
const currentSeason = (date = new Date(), hemisphere = 'north') => {
  const year = date.getFullYear()
  const yearDates = datesByYear(year)
  let season = 'winter'
  if (date > new Date(yearDates.spring) && date < new Date(yearDates.summer)) {
    season = 'spring'
  } else if (
    date > new Date(yearDates.summer) &&
    date < new Date(yearDates.fall)
  ) {
    season = 'summer'
  } else if (
    date > new Date(yearDates.fall) &&
    date < new Date(yearDates.winter)
  ) {
    season = 'fall'
  }

  if (hemisphere === 'south') {
    season = seasonByBemisphere[season]
  }
  return season
}
